...
descriptor = 'ECFP' # 'SMILES' or 'ECFP' Type of molecular descriptor. 
# If we want to use rnn architecture we use SMILES. Conversely, if we want to 
# use a fully connected architecture, we use ECFP descriptors. 
...
#    x_morg = morg_fp_np
#    
#    x_morg_rsmp, y_morg_rsmp = ADASYN().fit_resample(x_morg, labels_raw)
    ...
    if searchParameters:
    	# Split data into training, validation and testing sets. Oversampling
    ...
    if model_type == 'dnn' and descriptor == 'SMILES':
        ...
        data_rnn_ecfp = data_division(config,morg_fp_np,labels_raw,True,model_type,descriptor)
