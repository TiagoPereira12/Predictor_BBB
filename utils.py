...
def reading_csv(config,property_identifier):
    ...
    with open(filepath, 'r') as csvFile:
        ...
        for row in it:
            try:
                if "[S@H0]" not in row[idx_smiles] or "[S@@H0]" not in row[idx_smiles] or "[n+]" not in row[idx_smiles] or "[o+]" not in row[idx_smiles] or "[c@@]" not in row[idx_smiles]:
...
#    test_set =  smiles_int[idx_test]
    test_set = np.array(smiles_int)[idx_test.astype(int)]
    labels = np.array(labels)
    test_labels = labels[idx_test]

    x_morg = train_val_set
#    
#    x_morg_rsmp, y_morg_rsmp = ADASYN().fit_resample(x_morg, train_val_labels)
    x_morg_rsmp, y_morg_rsmp = SMOTE().fit_resample(x_morg, train_val_labels)
    ...
    if not cross_validation:
        ...
        data.append(x_morg_rsmp)
        data.append(y_morg_rsmp)
        data.append(test_set)
        data.append(test_labels)
